...
apex = [450, 290]           #for selecting the coordinate for middle point in triangle
...
for i in range(region_thresholds2.shape[0]):
    for j in range(region_thresholds2.shape[1]):
        if(region_thresholds2[i,j] == True):
            region_thresholds2[i,j] = False
        else:
            region_thresholds2[i,j] = True
...
threshold = (region_select[:,:,0]<rgb_threshold[0])|(region_select[:,:,1]<rgb_threshold[1])|(region_select[:,:,2]<rgb_threshold[2])
